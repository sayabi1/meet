  ...
  if (!accessToken || tokenCheck.error) {
    ...
    if (!code) {
      const results = await axios.get(
        "https://r80yj03wkk.execute-api.eu-central-1.amazonaws.com/dev/api/get-auth-url"
      );
...
export const getEvents = async () => {
  ...
  if (token) {
    removeQuery();
    const url =
      "https://r80yj03wkk.execute-api.eu-central-1.amazonaws.com/dev/api/get-events" +
      "/" +
      token;
